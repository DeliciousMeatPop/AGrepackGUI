...
[Files]
#if defined(IS_ENHANCED) && (VER < 0x05050100)
  #define DirExists(str S) \
    Local[0] = FindFirst(S, faDirectory), Local[0] > 0 ? FindClose(Local[0]), Local[0] : Local[0]
#endif
#define iFile 0
#define iCount 0
#define ModulePath ExtractFilePath(__PATHFILENAME__)
Source: "{#ModulePath}\English.ini"; DestDir: "{tmp}"; Flags: dontcopy
Source: "{#ModulePath}\Russian.ini"; DestDir: "{tmp}"; Flags: dontcopy
Source: "{#ModulePath}\UnArcLib.dll"; DestDir: "{tmp}"; Flags: dontcopy
Source: "{#ModulePath}\ISDone.dll"; DestDir: "{tmp}"; Flags: dontcopy
Source: "{#ModulePath}\SplitLib.dll"; DestDir: "{tmp}"; Flags: dontcopy
...
Source: "{#AddBackslash(SourcePath) + ExtractFileName(SetupSetting("OutputBaseFilename")) + ".dll"}"; DestDir: "{tmp}"; Flags: dontcopy
  ...
  #sub ProcessFoundFile
      #define DestPath (Trim(ExtractFilePath(StrFile)) != "") ? "{tmp}\" + ExtractFileDir(StrFile) : "{tmp}"
...
function DSG_pFindFilesEx(const FindPath, DestPath, FileMasks, ExcludeMasks: WideString; ResultKind: DSG_TFFResultKind; Recursive, HiddenFiles, SystemFiles, FindDirs, GetHash: Boolean): Longint; external 'pFindFilesEx@files:SplitLib.dll stdcall delayload';
...
function DSG_pPickFileList(const FindHandle: Longint): WideString; external 'pPickFileList@files:SplitLib.dll stdcall delayload';
...
//function DSG_pPickHash(const FindHandle: Longint; const Index: Integer): WideString; external 'pPickHash@files:SplitLib.dll stdcall delayload';
...
//function DSG_pPickFileFormat(const FindHandle: Longint; const Index: Integer): WideString; external 'pPickFileFormat@files:SplitLib.dll stdcall delayload';
//function DSG_pPickFileFormatList(const FindHandle: Longint): WideString; external 'pPickFileFormatList@files:SplitLib.dll stdcall delayload';
...
function DSG_pPickDirList(const FindHandle: Longint): WideString; external 'pPickDirList@files:SplitLib.dll stdcall delayload';
//function DSG_pPickDirFormat(const FindHandle: Longint; const Index: Integer): WideString; external 'pPickDirFormat@files:SplitLib.dll stdcall delayload';
//function DSG_pPickDirFormatList(const FindHandle: Longint): WideString; external 'pPickDirFormatList@files:SplitLib.dll stdcall delayload';
...
function DSG_FreeArcExtract(Callback: LongWord; Cmd1, Cmd2, Cmd3, Cmd4, Cmd5, Cmd6, Cmd7, Cmd8, Cmd9, Cmd10: PAnsiChar): Integer; external 'FreeArcExtract@files:UnArcLib.dll cdecl';
...
begin
  ...
  begin
    ...
    try
      ...
      Result := ErrorCode = 0;
      //UnloadDLL(ExpandConstant('{tmp}\UnArcLib.dll')); { error }
    end;
  end else
  begin
    Result := {#iCount} > 0;
...
begin
  ...
  hFind := DSG_pFindFilesEx(ExpandConstant('{tmp}'), '', '', '', dsg_ffrkRelative, True, True, True, False, False);
  try
    FileList := DSG_pPickFileList(hFind) + DSG_pPickDirList(hFind) + 'IsDone.dll' + #13#10 + 'UnArc.dll' + #13#10 + 'UnArcLib.dll' + #13#10 + 'CLS-DISKSPAN.dll';
  finally
    DSG_pFindFree(hFind);
  end;
  ...
  begin
        ...
        case Arcs[I].AType of
          ...
          tpDelta :
            begin
              ...
              begin
                hFind := DSG_pFindFiles(ExtractFilePath(DSG_ExpandConstantTry(Arcs[I].Source, 1)), ExtractFileName(Arcs[I].Source), '', dsg_ffrkRelative, True, False);
                try
                  SetArrayLength(PatchList, DSG_pFileCount(hFind));
                  for Y := 0 to DSG_pFileCount(hFind) - 1 do
                    PatchList[Y] := DSG_pPickFile(hFind, Y);
                finally
                  DSG_pFindFree(hFind);
                end;
              end else
    ...
    hFind := DSG_pFindFilesEx(ExpandConstant('{tmp}'), '', '', FileList, dsg_ffrkFull, True, True, True, False, False);
    try
      FileList := DSG_pPickFileList(hFind);
      for Y := 0 to DSG_pFileCount(hFind) - 1 do
      begin
        CurrItem := DSG_pPickFile(hFind, Y);
        if LowerCase(ExtractFileExt(CurrItem)) = '.exe' then
        begin
          DSG_CloseProcess(CurrItem);
          DeleteFile(CurrItem);
        end;
      end;
      (*
      ...
      begin
        ...
        begin
          while DSG_FreeLibrary(DSG_GetModuleHandle(CurrItem)) do {LOOP} ;
          UnloadDLL(CurrItem); { error }
        end;
        DeleteFile(CurrItem);
      end;
      *)
      for Y := 0 to DSG_pDirCount(hFind) - 1 do
      begin
        CurrItem := DSG_pPickDir(hFind, Y);
