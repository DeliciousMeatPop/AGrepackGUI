...
set COMMAND=call "%ARC_EXE_PATH%" create -cfg"%CONFIG_FILE_PATH%" -ep1 -r -s; --dirs -ds -w"%OUTPUT_DIRECTORY%" -m"srep_new+lolz:d55:mtt1:mt12:mc1023" -x"uninstall.exe" -x"uninstall.dat" -x"uninstall.bin" -x"unins???.exe" -x"unins???.dat" -dp"%REPACK_PATH%" "%DATA%"
